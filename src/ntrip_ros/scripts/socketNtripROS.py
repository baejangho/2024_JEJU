...
class SocketNtrip(object):
    def __init__(self, name):

        self.rtcm_topic = rospy.get_param("~rtcm_topic", "/rtcm")
        self.target_host = rospy.get_param("~target_host", "fkp.ngii.go.kr")
        self.target_port = rospy.get_param("~target_port", "2201")
        self.mountpoint = rospy.get_param("~mountpoint", "VRS_V32")
        self.useragent = rospy.get_param(
            "~useragent",
            "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/95.0.4638.69 Safari/537.36",
        )
        self.user = rospy.get_param("~user", "tge1375@naver.com:gnss")
...
if __name__ == "__main__":
    ...
    # try:
    #     while ntrip_ros_node.data and not rospy.is_shutdown():
    #         ntrip_ros_node.pubRTCM()
    #         rate.sleep()
    # except KeyboardInterrupt:
    #     ntrip_ros_node.force_stop = True
